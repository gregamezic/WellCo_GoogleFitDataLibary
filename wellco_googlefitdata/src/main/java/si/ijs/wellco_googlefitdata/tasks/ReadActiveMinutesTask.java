...
public class ReadActiveMinutesTask extends AsyncTask<Long, Long, Void> {


    private final String TAG = ReadActiveMinutesTask.class.getName();

    public OnActiveMinutesResult listener;
        ...
        if (dataReadResult.getBuckets().size() > 0) {
            ...
            for (Bucket bucket : dataReadResult.getBuckets()) {
                ...
                for (DataSet dataSet : dataSets) {
                    totalActiveTime += dataSet.getDataPoints().get(0).getValue(Field.FIELD_DURATION).asInt();
                }
            }
        }

        Log.d(TAG, "readDataResult: Total active time: " + totalActiveTime);

        //return result
        listener.onActiveMinutesDataResult(totalActiveTime);
